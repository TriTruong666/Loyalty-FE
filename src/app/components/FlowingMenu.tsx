...
const MenuItem: React.FC<MenuItemProps> = ({ link, text, image }) => {
  ...
  const handleMouseLeave = (ev: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    tl.to(marqueeRef.current, { y: edge === "top" ? "-101%" : "101%" })
      ...
      .set([marqueeRef.current, marqueeInnerRef.current], {
        clearProps: "all",
      });
  ...
  const repeatedMarqueeContent = React.useMemo(() => {
    return Array.from({ length: 4 }).map((_, idx) => (
      <React.Fragment key={idx}>
        ...
        <div className="w-[200px] h-[7vh] my-[2em] mx-[2vw] p-[1em_0] rounded-[50px] bg-white flex items-center justify-center overflow-hidden">
          <Image
            src={image}
            alt={text}
            width={200}
